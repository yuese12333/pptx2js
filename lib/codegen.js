...
function emitElement(el, slideVar, indent, options) {
  ...
  if (el.type === 'text') {
    const textOpts = [pos];
    if (el.bodyPr?.valign) {
      textOpts.push(`valign: '${escapeJs(el.bodyPr.valign)}'`);
    }
    if (el.bodyPr?.wrap === false) textOpts.push('wrap: false');
    return [
      `${indent(1)}${slideVar}.addText(${formatTextRuns(el.runs)}, { ${textOpts.join(', ')} });`,
    ];
  }
 ...
 * @param {object} opts
 */
function formatBorderSide(side) {
  if (!side) return "null";
  const parts = [`type: 'solid'`];
  if (side.color) parts.push(`color: '${escapeJs(side.color)}'`);
  if (side.pt != null) parts.push(`pt: ${side.pt}`);
  return `{ ${parts.join(', ')} }`;
}
...
function formatBorderObject(border) {
  ...
  if (sides.some((s) => border[s])) {
    return `{ ${sides.map((s) => `${s}: ${formatBorderSide(border[s])}`).join(', ')} }`;
  }
  return '{}';
}
...
function formatTextRuns(runs) {
  ...
  if (parts.length === 1 && formatRunOptions(runs[0].options ?? {}) === '{}') {
