...
function mergeTxBody(slideTxBody, layoutSp, masterSp) {
  ...
  if (!fallbackBody) return slideTxBody;

  const fallbackPara = children(fallbackBody, 'a:p')[0];
  const fallbackRPr =
    fallbackPara && child(children(fallbackPara, 'a:r')[0], 'a:rPr');
  if (!fallbackRPr) return slideTxBody;

  const merged = cloneNode(slideTxBody);
  for (const p of children(merged, 'a:p')) {
    for (const r of children(p, 'a:r')) {
      if (!child(r, 'a:rPr')) {
        r.children.push(cloneNode(fallbackRPr));
      }
    }
  }
  return merged;
